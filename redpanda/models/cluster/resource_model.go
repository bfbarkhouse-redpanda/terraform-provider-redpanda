...
func (r *ResourceModel) GetClusterCreate(ctx context.Context) (*controlplanev1.ClusterCreate, diag.Diagnostics) {
	...
	if !r.KafkaConnect.IsNull() {
		...
		output.KafkaConnect = connectSpec //nolint:staticcheck // SA1019: deprecated field still in use for backward compatibility
	}
...
func (r *ResourceModel) getClusterUpdate(ctx context.Context) (*controlplanev1.ClusterUpdate, diag.Diagnostics) {
	...
	if !r.KafkaConnect.IsNull() {
		...
		update.KafkaConnect = connectSpec //nolint:staticcheck // SA1019: deprecated field still in use for backward compatibility
	}
...
func (*ResourceModel) generateModelClusterConfiguration(cluster *controlplanev1.Cluster) (types.Object, diag.Diagnostics) {
	...
	if cfg.HasComputedProperties() {
		computedPropsMap := cfg.GetComputedProperties().AsMap()
		if len(computedPropsMap) > 0 {
			computedPropsBytes, err := json.Marshal(computedPropsMap)
			if err != nil {
				diags.AddError("failed to marshal computed properties", "could not convert computed properties to JSON")
				return types.ObjectNull(getClusterConfigurationType()), diags
			}
	...
	// Only return null if both custom and computed properties are null
	if configValues["custom_properties_json"].IsNull() && configValues["computed_properties_json"].IsNull() {
		return types.ObjectNull(getClusterConfigurationType()), diags
	}
